...
char* ler_arquivo(const char *arquivo) {
    ...
    if (buffer == NULL) {
        fprintf(stderr, "Falha ao alocar memória para ler o arquivo.\n");
        fclose(Arq);
        return NULL;
    }
    size_t bytes_lidos = fread(buffer, 1, tamanho, Arq);
...
void mostrar_alternativas(no* diretorio_atual, char* nome_parcial) {
    ...
    if (!encontrou_alternativas) {
        printf("Pastas disponiveis no diretorio atual:\n");
...
void comando_rm(char* arquivo, no* diretorio_atual) {
    ...
    while (*arquivo == ' ') arquivo++;
    char* fim = arquivo + strlen(arquivo) - 1;
    while (fim > arquivo && *fim == ' ') {
        *fim = '\0';
        fim--;
    }
    if (strlen(arquivo) == 0) {
    ...
    no* anterior = NULL;
    while (filho != NULL) {
        if (strcasecmp_custom(filho->nome, arquivo) == 0) {
...
void comando_mkdir(char* pasta, no* diretorio_atual) {
    ...
    while (*pasta == ' ' || *pasta == '\t') pasta++;
    char* fim = pasta + strlen(pasta) - 1;
    while (fim > pasta && (*fim == ' ' || *fim == '\t')) {
        *fim = '\0';
        fim--;
    }
    if (strlen(pasta) == 0) {
    ...
    for (int i = 0; pasta[i] != '\0'; i++) {
        if (pasta[i] == '/' || pasta[i] == '\\' || pasta[i] == ':' || 
            pasta[i] == '*' || pasta[i] == '?' || pasta[i] == '"' || 
            pasta[i] == '<' || pasta[i] == '>' || pasta[i] == '|') {
    ...
    if (strcmp(pasta, ".") == 0 || strcmp(pasta, "..") == 0) {
    ...
    no* filho = diretorio_atual->primeiroFilho;
    while (filho != NULL) {
        if (strcasecmp_custom(filho->nome, pasta) == 0) {
            if (filho->tipo == PASTA) {
    ...
    no* nova_pasta = criar_no(pasta, PASTA, diretorio_atual);
    if (nova_pasta == NULL) {
...
void comando_touch(char* arquivo, no* diretorio_atual) {
    ...
    while (*arquivo == ' ' || *arquivo == '\t') arquivo++;
    char* fim = arquivo + strlen(arquivo) - 1;
    while (fim > arquivo && (*fim == ' ' || *fim == '\t')) {
        *fim = '\0';
        fim--;
    }
    if (strlen(arquivo) == 0) {
    ...
    no* novo_arquivo = criar_no(arquivo, ARQUIVO, diretorio_atual);
...
void salvar_arvore_no_arquivo(no* raiz, const char* arquivo) {
    FILE* arquivo_ptr = fopen(arquivo, "w");
    if (arquivo_ptr == NULL) {
        printf("Erro: Não foi possível abrir o arquivo '%s' para escrita.\n", arquivo);
        return;
    }
    escrever_caminhos_recursivo(raiz, "/", arquivo_ptr);
    fclose(arquivo_ptr);
}
